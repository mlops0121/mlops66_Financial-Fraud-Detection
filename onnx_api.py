...
def root():
    ...
    return {"status": "running", "model_loaded": model_loaded, "backend": "ONNX Runtime"}
...
def predict(data: TransactionData):
    ...
    try:
        ...
        return PredictionResponse(
            fraud_probability=proba, is_fraud=proba >= 0.5, risk_level=get_risk_level(proba)
        )
...
def predict_batch(data: BatchTransactionData):
    ...
    try:
        ...
        predictions = [
            PredictionResponse(
                fraud_probability=float(p), is_fraud=p >= 0.5, risk_level=get_risk_level(p)
            )
            for p in probas
        ]
