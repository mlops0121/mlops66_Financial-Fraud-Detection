...
class UncertaintyAnalyzer:
    ...
    def analyze(self, proba, y_true=None, verbose=True):
        ...
        medium_risk = (proba >= self.thresholds["medium_low"]) & (
    ...
    def _print_threshold_analysis(self, proba, y_true):
        ...
        for threshold in [0.1, 0.2, 0.3, 0.5, 0.7, 0.8, 0.9]:
            ...
            print(
                f"{threshold:<10} {(tp + fp):<12,} {precision * 100:>7.1f}%   {recall * 100:>7.1f}%"
            )
    ...
    def get_risk_level(self, proba):
        ...
        risk_levels = np.where(
            proba >= self.thresholds["high_risk"],
            "HIGH",
            np.where(proba >= self.thresholds["medium_low"], "MEDIUM", "LOW"),
        )
        return risk_levels
