...
def test_optimize_memory_dtypes():
    ...
    assert df_opt['obj_col'].dtype == 'object', \
...
def test_find_latest_checkpoint_malformed_filenames(tmp_path):
    ...
    assert epoch == 1, \
        f"Should skip malformed filename and find epoch 1, got {epoch}"
