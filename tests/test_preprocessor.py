...
def test_split_data_time(preprocessor, sample_df):
    ...
    # Check shapes
    assert len(X_train) + len(X_valid) + len(X_test) == 100, "Total samples should match original data"
    assert len(y_train) + len(y_valid) + len(y_test) == 100, "Total labels should match original data"
...
def test_transform(mock_optimize, mock_extract_time, preprocessor, sample_df):
    ...
    assert isinstance(result['transaction_ids'], np.ndarray), "transaction_ids should be a numpy array"
...
def test_save_load(preprocessor, tmp_path):
    ...
    assert new_prep.drop_columns == ['col_test_drop'], "Loaded drop_columns should match saved state"
